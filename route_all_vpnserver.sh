...
Main_Menu() {
  ...
  printf '%b[4]%b = Install route_all_vpnserver.sh\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  printf '%b[5]%b = Install x3mRouting OpenVPN Event\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  printf '      ** Install Option 5 if you have installed Method 1 + Method 3\n'
  printf '%b[6]%b = Check for updates to existing x3mRouting installation\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  printf '%b[7]%b = Force update existing x3mRouting installation\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  printf '%b[8]%b = Remove x3mRouting Repository\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  localmd5="$(md5sum "/opt/bin/x3mRouting" | awk '{print $1}')"
  remotemd5="$(curl -fsL --retry 3 "${GITHUB_DIR}/x3mRouting" | md5sum | awk '{print $1}')"
  if [ "$localmd5" != "$remotemd5" ]; then
    printf '%b[9]%b = Update x3mRouting Menu\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  fi
  printf '\n%b[e]%b = Exit Script\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  while true; do
...
Install_Route_All_VpnServer() {
  Create_Project_Directory
  Download_File "$LOCAL_REPO" "route_all_vpnserver.sh"
  Check_OpenVPN_Event
  echo
  echo "Press enter to continue"
  read -r
...
Check_OpenVPN_Event() {
  ...
  PROJECT_DIR="/jffs/scripts/x3mRouting"
  OPENVPN_EVENT_FILE="openvpn-event"

  if [ -s "$PROJECT_DIR/$OPENVPN_EVENT_FILE" ]; then
    # file exists
    return
  else
    while true; do
      printf '\n\n%b%s%b file was not found.\n' "${COLOR_GREEN}" "$PROJECT_DIR/$OPENVPN_EVENT_FILE" "${COLOR_WHITE}"
      printf '\n\nThe openvpn-event file is required for route_all_server.sh script to work.\n'
      printf 'Would you like to download %b%s%b now?\n' "${COLOR_GREEN}" "$OPENVPN_EVENT_FILE" "${COLOR_WHITE}"
      printf '%b[1]%b  --> Yes\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
      printf '%b[2]%b  --> No\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
      echo
      printf '[1-2]: '
