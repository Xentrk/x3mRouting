...
# Extract IPs and search domains; write WINS
	...
	do
		...
		if "$option" | grep "dhcp-option WINS "; then "$option" | sed "s/ WINS /=44,/" >> "$conffile"; fi
		if "$option" | grep "dhcp-option DNS"; then serverips="$serverips $(echo "$option" | sed "s/dhcp-option DNS //")"; fi
		if "$option" | grep "dhcp-option DOMAIN"; then searchdomains="$searchdomains $(echo "$option" | sed "s/dhcp-option DOMAIN //")"; fi
	done
