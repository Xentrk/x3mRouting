...
create_client_list() {
  ...
  ## Xentrk: update vpnrouting.sh to use custom nvram file in /jffs/configs
  ...
  for ENTRY in $VPN_IP_LIST; do
    ...
    if [ "$VPN_IP" != "0.0.0.0" ]; then
      ...
      if [ "$TARGET_ROUTE" = "VPN" ]; then
        ...
        /usr/bin/logger -t "openvpn-updown-doug" "Forcing $VPN_IP to use DNS server $server"
      else
        echo /usr/sbin/iptables -t nat -I DNSVPN$instance -s $VPN_IP -j RETURN >>$dnsscript
        /usr/bin/logger -t "openvpn-updown-doug" "Excluding $VPN_IP from forced DNS routing"
      fi
    fi
  done
  IFS=$OLDIFS
...
if [ $script_type == 'up' ]; then
  ...
  if [ $instance != 0 -a $(nvram get vpn_client$(echo $instance)_rgw) -ge 2 -a $(nvram get vpn_client$(echo $instance)_adns) == 3 ]; then
    setdns=0
  else
    setdns=-1
  fi
...
if [ $script_type == 'down' -a $instance != 0 ]; then
...
if [ -f $conffile -o -f $resolvfile -o -n "$fileexists" ]; then
  if [ $script_type == 'up' ]; then
    if [ -f $dnsscript ]; then
      /bin/sh $dnsscript
    fi
