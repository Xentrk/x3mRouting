...
Main_Menu() {
  ...
  printf '%b[5]%b  Install route_ipset_vpnserver.sh\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  printf '%b[6]%b  Install x3mRouting OpenVPN Event\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  printf '      ** Install Option 6 if you have installed Method 1 + Method 3\n'
  printf '%b[7]%b  Check for updates to existing x3mRouting installation\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  printf '%b[8]%b  Force update existing x3mRouting installation\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  printf '%b[9]%b  Remove x3mRouting Repository\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  localmd5="$(md5sum "/opt/bin/x3mRouting" | awk '{print $1}')"
  remotemd5="$(curl -fsL --retry 3 "${GITHUB_DIR}/x3mRouting" | md5sum | awk '{print $1}')"
  if [ "$localmd5" != "$remotemd5" ]; then
    printf '%b[10]%b Update x3mRouting Menu\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  fi
  printf '\n%b[e]%b  Exit Script\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
  while true; do
...
Update_Version() {
  ...
  if [ -d "$DIR" ]; then
    for FILE in vpnrouting.sh \
      ...
      openvpn-event \
      route_all_vpnserver.sh \
      route_ipset_vpnserver.sh;do
      if [ -s "$DIR/$FILE" ]; then
        if [ -z "$1" ]; then
...
Install_Route_IPSET_Vpnserver() {
  ...
  Create_Project_Directory
  Download_File "$LOCAL_REPO" "route_ipset_vpnserver.sh"
  Check_OpenVPN_Event
  echo
  echo "Press enter to continue"
  read -r
